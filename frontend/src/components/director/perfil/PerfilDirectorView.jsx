...
const PerfilDirectorView = ({ director, onProfileUpdate }) => {
  ...
  const datosDirector = [
    { label: 'DNI', value: formData.dni || director?.dni || 'No disponible' },
    { label: 'Celular', value: formData.celular || director?.celular || 'No disponible' },
    { label: 'Correo', value: formData.email || director?.email || director?.correo || 'No disponible' },
    { label: 'RUC', value: formData.ruc || director?.ruc || director?.ruc_ie || director?.institucion_ruc || 'No registrado' },
  ];
        ...
        {editMode && (
          <form onSubmit={handleSubmit} className="mt-6 rounded-[24px] border border-slate-200 bg-slate-50 p-5 shadow-sm dark:border-slate-700 dark:bg-slate-900/60">
            ...
            <div className="mt-5 flex flex-wrap justify-end gap-3">
              <button type="button" onClick={() => setEditMode(false)} className="rounded-2xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-bold text-slate-700 transition hover:bg-slate-100 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-200 dark:hover:bg-slate-800">Cancelar</button>
